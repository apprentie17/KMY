...
if(isset($_POST['submit'])){
   ...
   if($old_pass == $empty_pass){
      ...
      $message[] = 'ancien mot de passe non correspondant!';
   }elseif($new_pass != $confirm_pass){
      $message[] = 'Confirmer le mot de passe non correspondant!';
   }else{
...
<section class="form-container">
   ...
   <form action="" method="post">
      <h3>mettre à jour le code</h3>
      <input type="hidden" name="prev_pass" value="<?= $fetch_profile['password']; ?>">
      <input type="text" name="name" value="<?= $fetch_profile['name']; ?>" required placeholder="Entrez votre nom d'utilisateur" maxlength="20"  class="box" oninput="this.value = this.value.replace(/\s/g, '')">
